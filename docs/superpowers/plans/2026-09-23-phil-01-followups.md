...
- **Child-process environment** (`src/phil/workspace/shell.py`). Commands inherit the full environment, including provider API keys, so agent-written test code can read them. Open decision for the user: scrub secrets by default or keep the inherited environment. This matters more than allowlist tuning, because running `pytest` already executes agent-written code (so `pytest -p <plugin>` and `--rootdir=/` are not meaningful boundaries).
